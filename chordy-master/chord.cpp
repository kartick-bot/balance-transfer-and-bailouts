...
void * chord::get_command(void * t)
{
	while(1)
	{
		...
		int idx_broadcast = cmd.find("Broadcast"); // Broadcast message by a node //LMS
		int idx_respond = cmd.find("Respond");//LMS
		int idx_rejoin = cmd.find("Recome");
		...
		
		{
			...
			start2 = clock();
			int node_id2 = 5;
			unicast_send(node_id, node_id2, "Broadcast");
		
			ofstream myfile;
 			myfile.open ("example02.txt");
  			myfile << "I am looking for incoming nodes for a 100 dollars with interest of 5% \n";
  			myfile.close();
			nd2 = clock();
			cout<<"time-broadcast"<<double(nd2-start2)/double(CLOCKS_PER_SEC) ;

		}
		else if(idx_respond >=0) //LMS
		{
			int node_idx = cmd.find_first_of(NUMBER);
			int node_id = atoi(&(cmd[node_idx]));
			int node_id2 = 2;
			unicast_send(node_id, node_id2, "Interested");
		}
		else if(idx_rejoin >=0) //LMS
		{
			clock_t start, end;
   			start = clock();
			int node_idx = cmd.find_first_of(NUMBER);
			...
			crash(node_id);
			cout<<"Node crashed, Please enter a new position and ID";
			int node_id2 = 11;
			//connect_to_peer(node_id,node_id2);
			join_one(node_id2);
			end = clock();
